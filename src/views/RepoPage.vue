<template>
    <main class="bg-[#B3D5F2]/20 dark:bg-gray-900">
      ...
      <section id="main-navigation" class="flex flex-col pt-20">
        <img :src="Gitwiki" alt="Gitwiki logo" class="w-32 md:w-48 self-center pb-10" />
        <SearchBar @handleSearch="handleSearch" :searchTerm="searchTerm" />
        <div v-if="filteredRepos.length === 0 && searchTerm" class="justify-center items-center h-screen flex flex-col">
      ...
      <section class="flex items-center justify-evenly dark:bg-gray-900 bg-[#B3D5F2]/20">
        <button @click="handle404Test" class="bg-[#D9042B]/80 dark:bg-white dark:text-[#D9042B]/80 text-white font-semibold p-3 mb-20 rounded-lg" aria-label="Test 404 Page">
          Test 404 Page
        </button>
        <button @click="handleErrorBoundaryTest" class="bg-[#D9042B]/80 dark:bg-white dark:text-[#D9042B]/80 text-white font-semibold p-3 mb-20 rounded-lg" aria-label="Test Error Boundary">
          Test Error Boundary
        </button>
      </section>
    </main>
  ...
  export default {
    ...
    // };
      ...
      const handleErrorBoundaryTest = () => {
        ...
        proxy.$emit('error', new Error('Test Error Boundary'));
      };
